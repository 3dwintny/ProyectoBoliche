        ...
        <div class="header-body">
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                    <h1 class="text-white">Editar práctica asignada</h1>
                </div>
            </div>
        </div>
    </div>
...
<div class="container-fluid pt-2">
    <div class="header-body text-center mb-7">
        <div class="row justify-content-center">
            ...
            @endif
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                <form method="POST" action="{{route('entreno-en-casa.update',encrypt($datosActividad->id))}}" enctype="multipart/form-data" role="form">
                    @csrf
                    {{method_field('PUT')}}
                    <input type="hidden" name="entrenador_id" id="entrenador_id" value="{{encrypt($datosActividad->entrenador_id)}}" required>
                    <div class="table-responsive">
                                ...
                                @else
                                    ...
                                    <tr class="table-sm">
                                        <td>
                                            ...
                                            <input type="text" value="{{$item->id}}" readonly style="text-align:center;width: {{$longitudContador}}em;border: none;">
                                        </td>
                                        <td>
                                            <input type="text" name="fecha[]" value="{{$datosActividad->fecha}}" id="fecha_registro{{$c}}" readonly style="width: 6em; text-align:center; border: none;" required>
                                        </td>
                                        <td>
                                            @php
                                        ...
                                        <td>
                                            <label>
                                                @if($item->id == $actividades[$controlAtletas])
                                                    <input type="checkbox" id="atletaSeleccionado{{$contador}}" class="{{$item->id}}" name="atletaSeleccionado[]"  value="{{encrypt($item->id)}}" checked>
                                                    @php
                                                        if($controlAtletas < count($actividades)-1){
                                                            $controlAtletas=$controlAtletas+1;
                                                        }
                                                    @endphp
                                                @else
                        ...
                        <textarea name="txtActividad" id="txtActividad" placeholder="{{ __('Actividad') }}" cols="40" rows="5" style="text-align:justify;resize:none;" class="form-control text-dark" aria-describedby="basic-addon2" required>{{$actividad}}</textarea>
                        @if(count($atletas)==0)
